			...
			<table id="tblSocket" width="100%" class="table table-striped table-bordered table-hover table-sm" style="width:100%">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{$tier->tiers}}</td>
						<td>
							<a href="{{ url('tiers',[$tier]) }}" class="btn btn-warning"><i class="fa-solid fa-edit"></i></a>
						</td>
						<td>
							<form method="POST" action="{{ url('tiers',[$tier]) }}">
								@method('delete')
								@csrf
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">NEW TIERS TAG</h5>
				...
				<div class="modal-body">
					<form id="formTag" method="POST" action="{{ url('tiers') }}">
						@csrf
						<div class="form-group">
							<label for="socket">TIERS</label>
							<input type="text" class="form-control" id="tiers" name="tiers" placeholder="Enter Tier" required>
						</div>
					</form>
				</div>
